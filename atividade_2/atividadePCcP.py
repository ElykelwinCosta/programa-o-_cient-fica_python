...
for name, value in zip(month_names, monthly_mean):
    ...
    if(value > maiorMedia):
        mesMaiorMedia = name
        maiorMedia = value
print("Mês com maior incidência de luz = %s: %.1f" %(mesMaiorMedia, maiorMedia))
...
for ano in data:
    ...
    anos += 1
    if(anos == 10):
        medias.append(["%.2f"%(somaJaneiro/10),"%.2f"%(somaDezembro/10)])
        decada += 1
        print("Média década {}: Janeiro = {} Dezembro = {}".format(decada, ("%.2f"%(somaJaneiro/10)), ("%.2f"%(somaDezembro/10))))
        somaJaneiro = 0
        somaDezembro = 0
        anos = 0
